...
import { UNITED_STATES_CONFIG } from '@/lib/constants/countries'
...
export default async function EsimPage({ params }) {
    ...
    // Fetch country information and United States bundles in parallel
    const [countryInfo, bundleData] = await Promise.all([
        getCountryInfo('united-states'),
        fetchUnitedStatesBundles()
    ]);
    ...
    const { countryName, flagUrl, heroImageUrl, description, bundleCountryInfo } = 
        prepareCountryData(UNITED_STATES_CONFIG, countryInfo, bundleData, slug);

    // Get network providers for United States
    const networkProviders = networkProvidersData[UNITED_STATES_CONFIG.networkProviders] || 
        networkProvidersData['united-states'] || 
        networkProvidersData['us'] || 
    ...
    return (
        <div className="min-h-screen">
            ...
            <EsimHeroSection
                ...
                trustpilotRating={UNITED_STATES_CONFIG.trustpilotRating}
                reviewCount={UNITED_STATES_CONFIG.reviewCount}
            />
